    ...
    function (CustomData, ovpLogger) {
        ...
        CustomData.prototype._checkWriteToDom = function (oRelated) {
            ...
            if (typeof value != "string") {
                oLogger.error("CustomData with key " + sKey + " should be written to HTML of " + oRelated + " but the value is not a string.");
                return null;
            }

            if (!(sap.ui.core.ID.isValid(sKey)) || (sKey.indexOf(":") != -1)) {
                oLogger.error("CustomData with key " + sKey + " should be written to HTML of " + oRelated + " but the key is not valid (must be a valid sap.ui.core.ID without any colon).");
                return null;
            }
            ...
            if (sKey == window._FASTNAVIGATIONKEY) {
                ...
                oLogger.error("CustomData with key " + sKey + " should be written to HTML of " + oRelated + " but the key is not valid (may not start with 'sap-ui').");
                return null;
            }
