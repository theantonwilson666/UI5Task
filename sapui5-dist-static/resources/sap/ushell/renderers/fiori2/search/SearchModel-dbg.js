        ...
        var SearchModel = sap.ui.model.json.JSONModel.extend("sap.ushell.renderers.fiori2.search.SearchModel", {
            ...
            _prepareTableResults: function (results) {
                ...
                for (i = 0; i < attributesInAll.length; i++) {
                    if (attributesInAll[i].usage.Detail &&
                        attributesInAll[i].group === undefined &&
                        attributesInAll[i].type !== "GeoJson" &&
                        attributesInAll[i].id.match(/latitude|longitude/i) == null &&
                        attributesInAll[i].type !== that.sinaNext.AttributeType.ImageUrl) { // filter out Andre's geo-attributes
            ...
            autoStartApp: function () {
                ...
                if (that.getProperty("/appCount") && that.getProperty("/appCount") === 1 && that.getProperty("/count") && that.getProperty("/count") === 1) {
                    ...
                    if (aApps && aApps.length > 0 && aApps[0] && aApps[0].url && that.getProperty('/uiFilter/searchTerm') &&
                        aApps[0].tooltip && that.getProperty('/uiFilter/searchTerm').toLowerCase().trim() === aApps[0].tooltip.toLowerCase().trim()) {
                        if (aApps[0].url[0] === '#') {
            ...
            calculateVisibility: function () {
                ...
                if (that.isAppCategory()) {
                    ...
                    if (!(resultToDisplay === "searchResultList" || resultToDisplay === "searchResultTable" || resultToDisplay === "searchResultGrid" || resultToDisplay === "searchResultMap")) {
                        that.setResultToDisplay("searchResultList");
                    }
