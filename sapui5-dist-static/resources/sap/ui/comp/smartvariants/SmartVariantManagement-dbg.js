...
sap.ui.define([
	...
	var SmartVariantManagement = SmartVariantManagementBase.extend("sap.ui.comp.smartvariants.SmartVariantManagement", /** @lends sap.ui.comp.smartvariants.SmartVariantManagement.prototype */
	{
		...
		renderer: function(oRm, oControl) {
			SmartVariantManagementBase.getMetadata().getRenderer().render(oRm, oControl);
		}
	});
		...
		if (oControl && FlexApplyAPI) {
			...
			this._oControlPromise = new Promise(function(resolve, reject) {
				...
				Promise.all([this._oMetadataPromise]).then(function() {
					...
					if (mPropertyBag.variants) {
						mPropertyBag.variants.forEach(function(oVariant) {
							this._aSpecialVariants.push(oVariant.id);
						}.bind(this));
					}
	...
	SmartVariantManagement.prototype._isSpecialVariantById = function(sKey) {
		return (this._aSpecialVariants.indexOf(sKey) >= 0);
	};
	...
	SmartVariantManagement.prototype._getVariantContent = function(sKey) {
		...
		if (oVariant && oVariant.getContent()) {
			var oTargetContent = {};
			var oSourceContent = oVariant.getContent();
			if (sKey === this.STANDARDVARIANTKEY) {
				oSourceContent = merge({}, this.getStandardVariant());
				oTargetContent = merge({}, oVariant.getContent());
			}

			return merge(oTargetContent, oSourceContent);
		} else { //TODO: remove once it is clear how to update the variant list
			var oChange = this._getChange(sKey);
			if (oChange) {
	...
	SmartVariantManagement.prototype._createVariantEntries = function(mVariants) {
		...
		if (mVariants) {
			...
			if (mVariants.standardVariant) {
				sStandardVariantKey = mVariants.standardVariant.getId();
				this._createVariantItem(mVariants.standardVariant);

				this._aSpecialVariants.push(sStandardVariantKey);
			}
		...
		if (this._oPersoControl) {
			...
			if (this._getIndustrySolutionMode()) {
				...
				if (sStandardVariantKey && (sStandardVariantKey !== this.STANDARDVARIANTKEY)) {
	...
	SmartVariantManagement.prototype._dataReceived = function(mVariants, bIsVariantSharingEnabled, bIsVariantCreationEnabled, bIsVariantAdaptationEnabled, oCurrentControlWrapper) {
		var oDefaultVariant, sKey, bFlag, parameter = {
			variantKeys: []
		};
		...
		if (!this._bIsInitialized) {
			...
			if (bFlag !== null) {
				this._executeOnSelectForStandardVariantByUser(bFlag ? true : false);
			}
	...
	SmartVariantManagement.prototype._updateVariant = function(oVariantInfo) {
		...
		if (this._isIndustrySolutionModeAndVendorLayer() || (oVariantInfo.key !== this.getStandardVariantKey())) {
			...
			if (oVariantInfo) {
				var oChange = this._getVariantById(oVariantInfo.key);
				if (oChange) {
					try {
		...
		if (oVariantInfo && FlexWriteAPI) {
			...
			return SyncPromise.resolve(this._fetchContentAsync()).then(function(oContent) {
				...
				if (oChange) {
					...
					// TODO:
					// TODO:
					//	the new variant has to be added to the list of variants !!!!!!!
					//
					//this._addToVariantsCollection(oChange);
			...
			for (i = 0; i < aVariantInfo.length; i++) {
				...
				if (aVariantInfo[i] === this.getStandardVariantKey()) {
					...
					if (!this._isIndustrySolutionModeAndVendorLayer()) {
						...
						//TODO: new standard in collection
					}
				}
				...
				if (oChange) {
					...
					//TODO: I do assume this is not required any more
					//this._removeFavoriteChangesForVariantId(aVariantInfo[i]);

					//this._removeFromVariantsCollection(aVariantInfo[i]);
				}
			}
		}
	};
	...
	SmartVariantManagement.prototype._getExecuteOnSelectOnStandardVariant = function() {
		...
		var bExecuteOnSelection = null;
		if (FlexApplyAPI) {
			bExecuteOnSelection = FlexApplyAPI.getExecuteOnSelect({control: this._oPersoControl});
		}
	...
	SmartVariantManagement.prototype._setExecuteOnSelectOnStandardVariant = function(bFlag) {
		...
		if (FlexWriteAPI) {
			FlexWriteAPI.setExecuteOnSelection({
				control: this._oPersoControl,
