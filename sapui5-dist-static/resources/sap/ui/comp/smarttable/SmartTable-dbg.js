...
sap.ui.define([
	...
	var SmartTable = VBox.extend("sap.ui.comp.smarttable.SmartTable", /** @lends sap.ui.comp.smarttable.SmartTable.prototype */ {
		metadata: {
			...
			properties: {
				 ...
				 * The calculated column widths can be minimum 3rem and maximum 20rem. To avoid the heuristic column width calculation for a particular column {@link #annotation:CssDefaults} annotation can be used.
				 *
				 ...
				 *  <li>Column resizing feature gets enabled for all columns, including custom columns, and the wrapping of column headers get disabled for the columns that are generated by the SmartTable itself.</li>
				 * </ul>
				 * Those properties must not be managed by the application.
				 *
				 * <b>Note:</b> This is not a dynamic property and cannot be changed once the control has been initialized. This feature has no effect on custom columns.
				 *
				 * @since 1.86
				 */
	...
	SmartTable.prototype.openDialogForKeyUser = function(sStyleClass, fCallBack) {
		...
		this._oPersController.openDialog({useAvailablePanels: true, styleClass: sStyleClass});
	};
	...
	SmartTable.prototype._createResponsivePopover = function(bResizeButton) {
		...
		if (bResizeButton) {
			this._oColumnHeaderPopover.addItem(new ColumnPopoverActionItem({
				...
				press: [function() {
					PluginBase.getPlugin(this._oTable, "sap.m.plugins.ColumnResizer").startResizing(this._oColumnClicked.getDomRef());
				}, this]
			}));
		}
	};
	...
	function measureText(sText, iMin) {
		...
		if (!SmartTable.oCanvasContext) {
			var fnUpdateFont = function() {
				SmartTable.oCanvasContext.font = parseFloat(ThemeParameters.get("sapMFontMediumSize")) * fBaseFontSize + "px " + ThemeParameters.get("sapUiFontFamily");
	 ...
	 * @param {float} [mConfig.padding=1] The additional width added to the content width calculation of the column to find the outer(API) width of the column
	...
	SmartTable.prototype._calcColumnWidth = function(oField, bAdditionalProperty, mConfig) {
		...
		if (bAdditionalProperty || !oField.additionalProperty || sDisplayBehaviour != "descriptionOnly") {
		...
		if (!bAdditionalProperty && sDisplayBehaviour != "idOnly" && oField.additionalProperty && this._oTableProvider && fWidth < iMax) {
		...
		if (mConfig.label) {
			...
			if (fWidth < iMax && fWidth < iLabelLength && iLabelLength > iMin) {
				...
				var fOrigLabelWidth = Math.min(measureText(sLabel) + mConfig.epsilon, iMax * 0.7);
		...
		fWidth = Math.min(fWidth, iMax) + fPadding;
	...
	SmartTable.prototype._updateVisibleColumnsWidthForEdit = function() {
		if (this._oTable && this.getEnableAutoColumnWidth()) {
			this._oTable.getColumns().forEach(function(oColumn) {
				if (oColumn.getVisible()) {
					...
					if (sColumnKey) {
						var oField = this._oTableProvider.getFieldMetadata(sColumnKey);
